...
if APPENDIX_HTML_PATH:
    ...
    _rels_xml = all_files[_rels_name].decode('utf-8')
    _existing_media = [n for n in all_files if n.startswith('word/media/')]
    doc_xml, _rels_xml, _appendix_media = splice_appendix(
        doc_xml, _rels_xml, APPENDIX_HTML_PATH, _existing_media,
        position=APPENDIX_POSITION, details_anchor='Application Details',
    )
    if _appendix_media:
        all_files[_rels_name] = _rels_xml.encode('utf-8')
        all_files[_ct_name] = ensure_image_content_types(
            all_files[_ct_name].decode('utf-8')
        ).encode('utf-8')
        all_files.update(_appendix_media)
